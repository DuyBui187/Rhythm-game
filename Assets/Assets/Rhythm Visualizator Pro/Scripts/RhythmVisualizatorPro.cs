...
public class RhythmVisualizatorPro : MonoBehaviour {
	...
	void CameraPosition () {
        if (visualization == Visualizations.Line) {
            ...
            cameraPos.z -= 170f;
            vfxCamera.transform.position = cameraPos;
            //cameraPos.y += 5f + height;
            //vfxCamera.transform.position = cameraPos;
            //vfxCamera.transform.LookAt(center);
            ...
            vfxCamera.transform.LookAt(soundBarsTransform.position);
    ...
    /// </summary>
    void CameraMovement () {
		vfxCamera.transform.position = center.position + (vfxCamera.transform.position - center.position).normalized * orbitDistance;

		if (rotateCamera)
			vfxCamera.transform.RotateAround (center.position, Vector3.up, -velocity * Time.deltaTime);
	}
