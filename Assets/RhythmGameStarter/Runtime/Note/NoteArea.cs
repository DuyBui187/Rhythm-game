...
namespace RhythmGameStarter
{
    public class NoteArea : MonoBehaviour
    {
            ...
            if (note)
            {
                switch (note.action)
                {
                    ...
                    case Note.NoteAction.LongPress:
                        ...
                        {
                            if (currentNote == null) // Bắt đầu LongPress
                            {
                                currentNote = note;
                                ...
                                longNoteDetecter.OnTouchDown();  // Bắt đầu LongPress
                                ...
                                // Đảm bảo gọi OnTouchHold() mỗi frame khi người chơi giữ
                                // Tạo sự kiện liên tục trong khi đang giữ để hệ thống nhận diện là vẫn đang long press
                                StartCoroutine(HandleLongPress());
                            }

                            // Kiểm tra nếu LongPress đã kết thúc
                            if (longNoteDetecter != null && longNoteDetecter.exitedLineArea)
                            {
                                ...
                                longNoteDetecter.OnTouchUp();  // Kết thúc quá trình LongPress
                                longNoteDetecter = null;
                                currentNote = null;
                            }
                        }
        ...
        private IEnumerator HandleLongPress()
        {
            ...
            {
                // Kiểm tra nếu note vẫn còn tương tác và chưa rời khỏi khu vực
                if (longNoteDetecter != null)
                {
                    longNoteDetecter.OnTouchHold();  // Gọi liên tục để duy trì trạng thái LongPress
                }
